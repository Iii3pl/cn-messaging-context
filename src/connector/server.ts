...
app.post("/workflows/summary-doc/publish", asyncRoute(async (req, res) => {
  const body = req.body as WorkflowRequest & WorkspaceWriteInput & {
    title?: string;
    current_user?: string;
...
app.post("/workflows/topic-map", asyncRoute(async (req, res) => {
  const body = req.body as WorkflowRequest & { max_topics?: number };
  const messages = await getWorkflowMessages(req, body);
...
app.post("/workflows/topic-thread", asyncRoute(async (req, res) => {
  const body = req.body as WorkflowRequest & { topic?: string; anchor_message_id?: string; window_size?: number };
...
app.post("/schedules/digest", asyncRoute(async (req, res) => {
  const body = req.body as WorkflowRequest & { scheduled_for?: string; title?: string };
...
app.post("/workspace/read", asyncRoute(async (req, res) => {
  const body = req.body as WorkspaceReadInput;
...
app.post("/workspace/write", asyncRoute(async (req, res) => {
  const body = req.body as WorkspaceWriteInput & { confirmed_by_user?: boolean; confirmation_summary?: string };
  if (!body.confirmed_by_user) {
    res.status(400).json({ error: "user_confirmation_required" });
    return;
  }
...
function formatDisplayTime(value: string | undefined): string | undefined {
  if (!value) {
    return undefined;
  }
...
function buildTopicMap(messages: MessageRecord[], maxTopics: number): Array<{
  topic: string;
  message_count: number;
  conversations: string[];
  latest_timestamp?: string;
  summary: string;
  sample_messages: Array<Pick<MessageRecord, "timestamp" | "sender" | "text" | "conversation_name" | "conversation_id">>;
}> {
...
function buildTopicThread(messages: MessageRecord[], topic: string, anchorMessageId: string | undefined, windowSize: number): string {
  if (messages.length === 0) {
...
async function appendScheduled(req: express.Request, input: Omit<ScheduledActionRecord, "id" | "created_at" | "status" | "tenant_id">): Promise<ScheduledActionRecord> {
...
function filterMessagesByTopic(messages: MessageRecord[], topic: string): MessageRecord[] {
  return messages.filter((message) => messageMatchesTopic(message, topic));
}

function messageMatchesTopic(message: MessageRecord, topic: string): boolean {
  const haystack = `${message.conversation_name ?? ""} ${message.text}`;
  if (haystack.includes(topic)) {
    return true;
  }
  const aliases: Array<{ pattern: RegExp; topic: RegExp }> = [
...
function optionalScheduleStatus(value: unknown): ScheduledActionRecord["status"] | undefined {
...
function optionalWritablePlatform(value: unknown): WritablePlatform | undefined {
...
function requireWritablePlatform(value: unknown): WritablePlatform {
...
function requireWorkspaceProvider(value: unknown): WorkspaceProvider {
...
function requireWorkspaceKind(value: unknown): WorkspaceResourceKind {
  if (
    value === "doc" ||
    value === "sheet" ||
    value === "base" ||
