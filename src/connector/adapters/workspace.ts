...
export interface WorkspaceStatus {
  feishu: { cli: "available" | "missing"; command: "lark-cli"; docs: boolean; sheets: boolean; base: boolean; whiteboard: boolean };
  dingtalk: { cli: "available" | "missing"; command: "dws"; doc: boolean; sheet: boolean; aitable: boolean };
  tencent: {
...
export async function readWorkspaceResource(input: WorkspaceReadInput): Promise<WorkspaceResourceResult> {
...
export async function writeWorkspaceResource(input: WorkspaceWriteInput): Promise<WorkspaceResourceResult> {
  if (input.dry_run) {
    return {
...
export async function listMentionMessages(input: MentionListInput): Promise<MentionStateResult> {
...
export async function listUnreadConversations(input: { platform: Platform; limit?: number }): Promise<MentionStateResult> {
...
export async function queryMessageReadStatus(input: ReadStatusInput): Promise<MentionStateResult> {
  if (input.platform === "dingtalk") {
...
async function writeFeishuWorkspace(input: WorkspaceWriteInput): Promise<WorkspaceResourceResult> {
  if (input.kind === "doc" || input.kind === "smartcanvas") {
...
async function readDingTalkWorkspace(input: WorkspaceReadInput): Promise<WorkspaceResourceResult> {
  if (input.kind === "doc" || input.kind === "smartcanvas") {
...
async function writeDingTalkWorkspace(input: WorkspaceWriteInput): Promise<WorkspaceResourceResult> {
  if (input.kind === "doc" || input.kind === "smartcanvas") {
    const raw = input.mode === "create" || !input.target
      ? await runJson("dws", [
          "doc",
...
async function readTencentWorkspace(input: WorkspaceReadInput): Promise<WorkspaceResourceResult> {
...
async function writeTencentWorkspace(input: WorkspaceWriteInput): Promise<WorkspaceResourceResult> {
...
async function callTencentDocsApi(method: string, path: string, body: unknown): Promise<unknown> {
...
function unsupported(input: WorkspaceReadInput, action: WorkspaceResourceResult["action"], diagnostic: string): WorkspaceResourceResult {
  return {
    provider: input.provider,
    kind: input.kind,
    action,
...
function adapterName(provider: WorkspaceProvider): string {
  return provider === "feishu" ? "lark-cli" : provider === "dingtalk" ? "dws" : "tencent-docs-openapi";
}

function tencentApiBase(): string {
...
function requiredArray<T>(value: T[] | undefined, name: string): T[] {
...
function requiredMatrix(value: unknown[][] | undefined, name: string): unknown[][] {
  if (!value || value.length === 0) {
    throw new Error(`${name} is required`);
  }
  return value;
...
function pushTargetArg(args: string[], target: string | undefined): void {
...
function optionalParentArgs(input: WorkspaceWriteInput): string[] {
  if (input.parent_id) {
...
function optionalDingTalkParentArgs(input: WorkspaceWriteInput): string[] {
  if (input.parent_id) {
...
function matrixToCsv(rows: unknown[][]): string {
...
function run(command: string, args: string[], options: { timeoutMs: number }): Promise<string> {
