...
├── direct-approve.mjs← 🆕 直达详情页审批；跳过列表页，URL 直连 APPROVE_PEND 视口
...
#### 5.1 直达审批（direct-approve.mjs）🆕

**适用场景**：用户已明确给出 billId 列表，快速批量通过。

与 `fast-approve.mjs`（行级列表按钮）不同，`direct-approve.mjs` **直接构造详情页 URL 打开审批视口**：

```bash
# 预览模式（不执行）
node scripts/direct-approve.mjs --ids 2026062565792104,2026063067262409

# 执行
node scripts/direct-approve.mjs --ids 2026062565792104,2026063067262409 --yes
```

**核心优势**（2026-06-30 18 单实战验证）：

| 对比 | fast-approve.mjs | direct-approve.mjs |
|------|-----------------|-------------------|
| 导航方式 | 列表页 → 逐行定位 row → 点击行内按钮 | 直接 `?billId=XXX&viewType=APPROVE_PEND` |
| 幽灵单据 | 需要 ghost-clear 预处理 | `APPROVE_PEND` 视口绕过列表 tab 不一致 |
| 按钮定位 | 行内 button 查找（受行展开/折叠影响） | 详情页 `ant-btn-primary` + 文本"通过" |
| 失败模式 | ROW_NOT_FOUND / BUTTON_NOT_FOUND | PAGE_NOT_LOADED（可重试） |

**URL 公式**：
```
https://xft.cmbchina.com/TripMainWeb/#/trip-app/billDetail?billId={单号}&viewType=APPROVE_PEND&reserveTab=true
```

关键参数：
- `viewType=APPROVE_PEND` — 强制进入审批视口（即使单据在列表「已审批」tab 也不影响）
- `reserveTab=true` — 保留审批 tab 状态，避免 SPA 默认跳回列表

**Agent 使用指南**：
1. 收到用户指定 billId 列表后，优先用 `direct-approve.mjs --ids` 
2. `--dry-run`（默认）先验证每笔单据能打开 → 用户确认后加 `--yes` 执行
3. 按钮定位策略：`ant-btn-primary` + 精确文本「通过」→ sleep 2s → modal 内「确认」
